...
レベルのメッシュは床だけを残し、図のように上側の壁、下側の壁、`PlayerStart`を配置します。
...
`AlertCollision`は、図のように床の横幅いっぱいまで広げます。
...
`BP_AlertZone`を、`PlayerStart`の少し先に配置します。左右どちらへ進む場合も、先に警戒エリアを通る位置にしてください。
